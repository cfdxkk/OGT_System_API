...
import OGTSystem.util.BaiduCloudTextReview;
...
public class GroupMessageService {
    ...
    // 创建线程安全的 BaiduCloudTextReview 对象
    private static BaiduCloudTextReview baiducloudtextreview;
    @Autowired
    public void setBaiducloudtextreview(BaiduCloudTextReview baiducloudtextreview){
        GroupMessageService.baiducloudtextreview = baiducloudtextreview;
    }
    ...
    public boolean MessageToGroupUser (GroupMessageVo groupmessagevo) {
        ...
        if (grouprelationshipservice.checkUserInGroup(groupmessagevo.getUuidFrom(), groupmessagevo.getGroupIdFrom())) {
            ...
            // 2. 检查消息是否违规 (事件不检查)
            if( !("1".equals(groupmessagevo.getMessageType())) || baiducloudtextreview.BaiduCloudTextReview(groupmessagevo.getMessage())) {
